...
$sql_table_body = "SELECT * FROM tbl_used_material ORDER BY id DESC";
...
if (mysqli_num_rows($query_table_body) > 0) {
    while ($row_table_body = mysqli_fetch_array($query_table_body)) {
        ...
        $table_inventories .= '
            <tr>
                <td><a id="td-' . $row_table_body["inv_used_id"] . '">' . $row_table_body["inv_used_id"] . '<a></td>
                <td>' . $row_dealer_table_body["dealer_name"] . '</td>
                <td>' . $row_table_body["inv_used_time"] . '</td>
                <td>' . $row_table_body["materials_updated"] . '</td>
                <td>' . $row_table_body["quantity_updated"] . '</td>
                <td>' . $row_table_body["inv_used_note"] . '</td>
                <td>
                    <div class="buttons">
    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>ALL USED MATERIAL INVENTORIES</h3>
            </div>
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class='breadcrumb-header'>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="index.html">Admin</a></li>
                        <li class="breadcrumb-item active" aria-current="page">All used material inventories</li>
                    </ol>
                </nav>
            </div>
        </div>
                            ...
                            <div class="modal-body">
                                    <div class="row mb-4">
                                        <div class="col-md-8">
                                                <div class="card-body">
                                                    <div class="card-header">
                                                        <h4 class='card-heading'>USED QUANTITY</h4>
                                                    </div>
...
?>
                                        ...
                                        <div class="col-md-4">
                                                ...
                                                <div class="card-body">
                                                    <form role="form" method="post" enctype="multipart/form-data" id="form_input_inv_used">
                                                            <div class="row">
                                                                <div class="card-header">
                                                                    <h4>ID <strong style="color: #5A8DEE"><small id="inv_used_id_text"><!--inv_used_id--></small></strong></h4>
                                                                </div>

                                                                <div id="inv_used_id_input"><!--inv_used_id_input--></div>
...
?>
                                                                ...
                                                                <div class="form-group" id="picker_date">
                                                                    <label for="" class="form-label">TIME</label>
                                                                    <div class="position-relative">
                                                                        <input type="text" class="form-control datepicker-here" placeholder="Time" id="inv_used_time" name="inv_used_time" required>
                                                                    </div>
                                                                </div>

                                                                <div class="form-group">
                                                                    <label for="" class="form-label">NOTE</label>
                                                                    <textarea maxlength="150" class="form-control" id="inv_used_note" rows="3" name="inv_used_note" placeholder="Note"></textarea>
                                                                </div>
                                        ...
                                        <div class="col-12 d-flex justify-content-end ">
                                            <button class="btn btn-primary mr-1 mb-1" onclick="add_inv_used()">Add</button>
                                            <button class="btn btn-light-secondary mr-1 mb-1" data-dismiss="modal" >Cancel</button>
                                        </div>
                                    </div>
                                </div>
...
function show_add_modal()
{
    if (isset($_GET["name_modal"])) {
        if ($_GET["name_modal"] == "add_material_inv_used") {
            echo "
...
?>
        ...
        function create_id(){
            var inv_used_id_text = $.now().toString().slice(3);
            $('#inv_used_id_text').html("#"+inv_used_id_text);
            var inv_used_id_input = '<input type="hidden" value="' + inv_used_id_text + '" id="inv_used_id" name="inv_used_id" required>'
            $('#inv_used_id_input').html(inv_used_id_input);
        };
        ...
        function add_inv_used(){

            var inv_used_id = $('#inv_used_id').val();
            var dealer_id = $('#dealer_id').val();
            var inv_used_time = $('#inv_used_time').val();
            var materials_updated = $('#materials_updated').text();
            var quantity_updated = $('#quantity_updated').text();

            if($('#inv_used_note').val()){
                var inv_used_note = $('#inv_used_note').val();
            } else {
                var inv_used_note = "Nothing";
            }
            var action = "add";
            ...
            if( !dealer_id
                ...
                Toastify({
                    ...
                    form_data.append("inv_used_id", inv_used_id);
                    form_data.append("dealer_id", dealer_id);
                    form_data.append("inv_used_time", inv_used_time);
                    form_data.append("inv_used_note", inv_used_note);
                    form_data.append("materials_updated", materials_updated);
                    form_data.append("quantity_updated", quantity_updated);
                    form_data.append("action", action);

                    $.ajax({
                        url: "action-for-material-inventory-used.php",
                        type: "POST",
                        dataType: 'script',
                        cache: false,
                        contentType: false,
        ...
        function load_table_data(){
            $.ajax({
                url: "get-table-for-material-used.php",
                type: "POST",
                success: function(data){
                    $('#all_items').html(data);
                }
        ...
        function empty_table(){
            var action = "empty";
            $.ajax({
                url: "action-for-material-table-used.php",
                type: "POST",
                data: {action: action},
                success: function(){
                    Toastify({
        ...
        function update_table_item_quantity(id){
            var used_quantity = $("#input_material_quantity"+id).val();
            var action = "update";
            var material_id = id;
        ...
        $("#select_material").change(function(){
            ...
            form_data.append("used_quantity", used_quantity);
            form_data.append("action", action);
